...
class Core // Heart of the HTML Compressor.
{
	...
	public function __construct(array $options = array())
	{
		...
		else $this->regex_vendor_css_prefixes = implode('|', $this->pregQuoteDeep($this->default_vendor_css_prefixes, '/'));
		...
		if(isset($this->options['regex_css_exclusions']) && is_string($this->options['regex_css_exclusions']))
			...
			$this->regex_css_exclusions = ($this->options['css_exclusions']) // Contains exclusions?
				? '/'.implode('|', $this->pregQuoteDeep($this->options['css_exclusions'], '/')).'/i'
		...
		if(isset($this->options['regex_js_exclusions']) && is_string($this->options['regex_js_exclusions']))
			...
			$this->regex_js_exclusions = ($this->options['js_exclusions']) // Contains exclusions?
				? '/'.implode('|', $this->pregQuoteDeep($this->options['js_exclusions'], '/')).'/i'
				: ''; // No exclusions; i.e. pass an empty array to indicate none.
		...
		require_once dirname(dirname(__FILE__)).'/vendor/websharks/js-minifier/includes/classes/Core.php';
	}
	...
	public function compress($input)
	{
		...
		if(!isset($this->options['cleanup_cache_dirs']) || $this->options['cleanup_cache_dirs'])
			if(mt_rand(1, 20) === 1) $this->cleanupCacheDirs();
		...
		if(($html_frag = $this->getHtmlFrag($html)) && ($head_frag = $this->getHeadFrag($html)))
			if(($css_tag_frags = $this->getCssTagFrags($html_frag)) && ($css_parts = $this->compileCssTagFragsIntoParts($css_tag_frags)))
			{
				...
				if($benchmark) // Record benchmark data?
					$this->benchmark->addData( // For debugging.
	...
	protected function getCssTagFrags(array $html_frag)
	{
		...
		if(!empty($html_frag['contents']) && preg_match_all($regex, $html_frag['contents'], $_tag_frags, PREG_SET_ORDER))
		{
			foreach($_tag_frags as $_tag_frag)
			{
				...
				else if(($_style_css = $this->getStyleCss($_tag_frag, TRUE)))
	...
	protected function resolveResolvedCssImports($css, $media, $___recursion = FALSE)
	{
		...
		if(preg_match_all($import_media_without_url_regex, $css, $_m))
			foreach($_m['media'] as $_media) if(!$_media || $_media === $this->current_css_media)
				return $this->resolveResolvedCssImports($css, $this->current_css_media, TRUE); // Recursive.
	...
	protected function getJsTagFrags(array $html_frag)
	{
		...
		if(!empty($html_frag['contents']) && preg_match_all($regex, $html_frag['contents'], $_tag_frags, PREG_SET_ORDER))
		{
			foreach($_tag_frags as $_tag_frag)
			{
				...
				if($this->isScriptTagFragJs($_tag_frag)) // Check this first (for optimization).
	...
	protected function maybeCompressCssCode($css)
	{
		...
		if(!$css || !empty($disabled)) goto finale; // Nothing to do.
	...
	protected function cacheDir($type, $checksum = '', $base_only = FALSE)
	{
		...
		if(!empty($this->options[__FUNCTION__.'_'.$type]))
			$basedir = $this->nDirSeps($this->options[__FUNCTION__.'_'.$type]);
	...
	protected function cacheDirUrl($type, $checksum = '', $base_only = FALSE)
	{
		...
		if(!empty($this->options[__FUNCTION__.'_'.$type]))
			$baseurl = $this->setUrlScheme(rtrim($this->options[__FUNCTION__.'_'.$type], '/'));

		else if(defined('WP_CONTENT_DIR') && defined('WP_CONTENT_URL') && $basedir === $this->nDirSeps(WP_CONTENT_DIR.'/htmlc/cache/'.$type))
	...
	protected function currentUrlScheme()
	{
		...
		if(!empty($this->options[__FUNCTION__])) // Defined explicity?
			return (static::$static[__FUNCTION__] = $this->nUrlScheme($this->options[__FUNCTION__]));
	...
	protected function currentUrlHost()
	{
		...
		if(!empty($this->options[__FUNCTION__])) // Defined explicity?
			return (static::$static[__FUNCTION__] = $this->nUrlHost($this->options[__FUNCTION__]));
	...
	protected function currentUrlUri()
	{
		...
		if(!empty($this->options[__FUNCTION__])) // Defined explicity?
			return (static::$static[__FUNCTION__] = $this->mustParseUri($this->options[__FUNCTION__]));
	...
	protected function setUrlScheme($url, $scheme = '')
	{
		...
		if($scheme !== '//') $scheme = $this->nUrlScheme($scheme).'://';
	...
	protected function resolveRelativeUrl($relative_url_uri_query_fragment, $base_url = '')
	{
		...
		if(!$base_url) // No base URL? The `$base` is optional (defaults to current URL).
			$base_url = $this->currentUrl(); // Auto-detects current URL/location.
	                          ...
	                          $cookie_file = '', $fail_on_error = TRUE, $return_array = FALSE)
	{
		...
		if($benchmark) // Record benchmark data?
			$this->benchmark->addData( // For debugging purposes.
